...
func (sg *scenarioGroup) populateControlPositionsFromFacilities(e *util.ErrorLogger) {
	...
	for facilityID, facility := range sg.Facilities {
		...
		for _, pos := range facility.ControlPositions {
			...
			routingID := pos.SectorRoutingID
			if idx := strings.LastIndex(routingID, "_"); idx != -1 {
				routingID = routingID[idx+1:]
			}
			routingID = strings.TrimSpace(routingID)
			if routingID == "" {
				e.ErrorString("\"sector_routing_id\" must include a routing identifier")
				e.Pop()
				continue
			}
			...
			case av.FacilityTypeARTCC:
				...
				if facility.AbbreviatedFacilityID != "" {
					routing = strings.TrimPrefix(routingID, facility.AbbreviatedFacilityID)
					if routing == "" {
						routing = routingID
					}
				}
				if routing == "" {
					e.ErrorString("ARTCC control positions require a numeric routing identifier")
					e.Pop()
					continue
				}
				ctrl.ERAMFacility = true
				ctrl.FacilityIdentifier = facility.AbbreviatedFacilityID
				ctrl.TCP = facility.AbbreviatedFacilityID + routing
				legacyID = ctrl.TCP
				routingID = routing
			...
			case av.FacilityTypeAdjacentTRACON:
				...
				if facility.AdjacentTRACONID != "" {
					routing = strings.TrimPrefix(routingID, facility.AdjacentTRACONID)
					if routing == "" {
						routing = routingID
					}
				}
				if routing == "" {
					e.ErrorString("Adjacent TRACON control positions require a routing identifier")
					e.Pop()
					continue
				}
				ctrl.FacilityIdentifier = facility.AdjacentTRACONID
				ctrl.TCP = routing
				legacyID = facility.AdjacentTRACONID + routing
				routingID = routing
			default:
				ctrl.TCP = routingID
				legacyID = ctrl.Id()
...
func (sg *scenarioGroup) PostDeserialize(e *util.ErrorLogger, simConfigurations map[string]map[string]*Configuration,
	...
	sg.FacilityAdaptation.PostDeserialize(sg, sg.Facilities, controlledAirports, allAirports, e)
